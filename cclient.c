...
#define maxbuf 1024 // use preprocessor constant so it can be used in the typedef

static const int MAXBUF = maxbuf; // max size of message received from client
static const int MAXARGBUF = 32; // max size of parameters for host, port.
const static int default_threads = 1000;
static int requests;
...
//server_message prepare_server_message(addy, req_buf);
...
void *do_client_thread(void *arg) {
  ...
  sprintf(req_buf, "arijort:omet dadage do %ld\n", pthread_self());
